...
class Flickr8k(data.Dataset):
    def __init__(self, root='Flickr8k', train=True, transform=None):
        super().__init__()

        self.path = {'imgs': os.path.join(root, 'images'),
                      'captions': os.path.join(root, 'data', 'Flickr8k.token.txt'),}
        if train:
            self.path['ids'] = os.path.join(root, 'data', 'Flickr_8k.trainImages.txt')
        else:
            self.path['ids'] = os.path.join(root, 'data', 'Flickr_8k.testImages.txt')
